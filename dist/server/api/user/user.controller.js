...
function followUser(req, res) {
  ...
  var currentUserFeed = client.feed('user', req.user._id);

  _userModel2['default'].findByIdAsync(req.user._id).then(handleEntityNotFound(res)).then(saveUpdates(req.body.user)).then(function (doc) {
...
function me(req, res, next) {
  ...
  // Connect to Stream feed
  var currentUserFeed = client.feed('user', userId);
  var response = {
    user: null,
    followers: null,
    following: null,
